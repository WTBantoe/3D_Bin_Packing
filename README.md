...
- 在put函数中，由于存在strict_level的检查，因此会出现悬空不可放置而错过部分候选点的荒谬情况，因此准备添加箱子的“下落”功能，增加可利用的空间
- 目前的箱子在放入之前被旋转的方式是固定的，不能根据放置情况改变，这是不合理的，可以从这一方面改进
- 仅在候选点算法中存在try_rotate参数，可以在其它方法中进行添加
- 目前的算法虽然可以通过参数的组合以达到真实的从里到外、从下到上的放置顺序，但没有强制规定，可以在放置时进行限制
- 已经出现了许多装箱问题的高效算法，可以进行复现。其中，最近还出现利用神经网络进行求解该问题的算法，个人较感兴趣
